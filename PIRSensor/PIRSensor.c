 ...
 * LOCAL VARIABLES
 */
static uint8 PIRSensor_TaskID;   // Task ID for internal task/event processing
...
#if defined( CC2540_MINIDK )
static void PIRSensor_HandleKeys( uint8 shift, uint8 keys );
#endif
...
// GAP Role Callbacks
static gapRolesCBs_t PIRSensor_PeripheralCBs =
{
...
static gapBondCBs_t PIRSensor_BondMgrCBs =
{
...
static void PIRSensor_ProcessOSALMsg( osal_event_hdr_t *pMsg )
{
  ...
  #if defined( CC2540_MINIDK )
    case KEY_CHANGE:
      PIRSensor_HandleKeys( ((keyChange_t *)pMsg)->state, ((keyChange_t *)pMsg)->keys );
      break;
